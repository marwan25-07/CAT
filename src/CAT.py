...
class vae(nn.Module):
    def __init__(self,solvent_library=None, model_weights=None, scaler_weights=None, input_dim = 1621, hidden_dim = 1024, hidden_dim1 = 512, latent_dim = 256):
        ...
        if model_weights is not None:
            state_dict = joblib.load(model_weights)
            self.load_state_dict(state_dict)

        if solvent_library is not None:
            self.solvent_df = pd.read_csv(solvent_library)
            solvent_df = self.solvent_df
            self.solvent_features = solvent_df.iloc[:,2:]

        if scaler_weights is not None:
            self.scaler = joblib.load(scaler_weights)
    ...
    def forward(self,x, sample_reaction):
        solvent_features = self.solvent_features
    ...
    def optimise(self, reaction_sample):
        ...
        solvent_df = self.solvent_df 
        solvent_features= self.solvent_features
        reactant = reaction_sample.split('>>')[0] #define reactants 
        ...
        best_solvent = self.solvent_df['Solvent'].loc[best_index]
        return f'Best Solvent: {best_solvent}'
